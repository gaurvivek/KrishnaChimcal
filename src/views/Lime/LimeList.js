...
import CreateCoalEntry from './CreateLimeEntry';
...
const LimeListManage = () => {
  ...
  return (
    <Grid container spacing={2}>
      <CreateCoalEntry title={'Add Lime Entry'} editCoalItem={editCoalItem} open={showCoalManageDialog} onClose={onClose} onConfirm={onConfirm} />
      <ConfirmDialog title="Delete Lime Entry" text={<span>Do you want to delete <b>{deleteCoalItem.cName}</b> entry ?</span>} open={openDeleteConfirm} onClose={onClose} onConfirm={handleDeleteCoalItem}/>
      <AppSnackBarAlert
          key="2"
          open={hideConfirmSuccess}
          message={'Lime Entry Deleted Successfully...'}
          severity="success"
          onClose={onClose}
        />
      <Grid item xs={12}>
        <AppSection title="Lime Managment">
          <AppButton startIcon="add" label="Add Lime Entry" color="primary" onClick={handleCreateEntry}/>
          <Box sx={{ width: '100%' }}>
